    ...
    def __init__(self,config):
        ...
        with tf.variable_scope("Net",initializer=tf.orthogonal_initializer()):
            def lstm_cell():
                ...
                return tf.nn.rnn_cell.BasicLSTMCell(self.hidden_size, forget_bias=1.0, state_is_tuple=True)
            ...
            if self.dropout_keep_prob is not None:
                def attn_cell():
                    ...
                    return tf.nn.rnn_cell.DropoutWrapper(lstm_cell(), output_keep_prob=self.dropout_keep_prob)

            self.cell_fw=tf.nn.rnn_cell.MultiRNNCell([attn_cell() for _ in range(config.num_layers)],
                                                     state_is_tuple=True)
            self.cell_bw=tf.nn.rnn_cell.MultiRNNCell([attn_cell() for _ in range(config.num_layers)],
                                                     state_is_tuple=True)
            ...
            with tf.name_scope("train_op"):
                ...
                self.l2_loss = 0.001 * tf.reduce_sum([tf.nn.l2_loss(v) for v in tvars])
